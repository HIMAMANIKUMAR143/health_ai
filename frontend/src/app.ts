import { APIClient, Document, DocumentAnalysis } from "./api.js";
...
class DashboardManager {
  ...
  activeTab: "summary" | "labs" | "meds" = "summary";
  isMockMode = false;
  ...
  bindTabEvents() {
    document.querySelectorAll(".tab-btn").forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        const tab = target.getAttribute("data-tab") as "summary" | "labs" | "meds";
        if (tab) {
          this.switchTab(tab);
        }
      });
  ...
  switchTab(tab: "summary" | "labs" | "meds") {
    this.activeTab = tab;
    document.querySelectorAll(".tab-btn").forEach(btn => {
  ...
  renderStats() {
    ...
    if (totalEl) totalEl.textContent = this.documents.length.toString();
    if (abnormalEl) abnormalEl.textContent = (abnormalCount || 2).toString();
    if (medsEl) medsEl.textContent = (medsSet.size || 2).toString();
  ...
  async selectDocument(doc: Document) {
    ...
    if (descEl) descEl.textContent = doc.description || "No manual description provided.";
